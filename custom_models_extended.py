...
def initialize_regression_model(input_dim, hidden_channels=[64], gcn_layers=4, linear_sizes=[], aggregations=[global_mean_pool, global_max_pool], apply_random_aggregations=False, learning_rate=0.001, permute_index=[], pre_act_permute_gcn=False, post_act_permute_gcn=False):

    model = GCN_molecule_regression(input_dim = input_dim, hidden_channels=hidden_channels, gcn_layers=gcn_layers, aggregations=aggregations, apply_random_aggregations=apply_random_aggregations, permute_index=permute_index, pre_act_permute_gcn=pre_act_permute_gcn, post_act_permute_gcn=post_act_permute_gcn)
    print(model)
...
def initialize_classification_model(input_dim, hidden_channels=[64], gcn_layers=4, linear_sizes=[], aggregations=[global_mean_pool, global_max_pool], apply_random_aggregations=False, learning_rate=0.001, permute_index=[], pre_act_permute_gcn=False, post_act_permute_gcn=False):

    model = GCN_molecule_classification(input_dim = input_dim, hidden_channels=hidden_channels, gcn_layers=gcn_layers, aggregations=aggregations, apply_random_aggregations=apply_random_aggregations, permute_index=permute_index, pre_act_permute_gcn=pre_act_permute_gcn, post_act_permute_gcn=post_act_permute_gcn)
    print(model)
    print("Number of parameters: ", sum(p.numel() for p in model.parameters()))
...
class GCN_molecule_regression(torch.nn.Module):
    ...
    def __init__(self, input_dim, hidden_channels=[64], gcn_layers=4, linear_sizes=[], aggregations=[global_mean_pool, global_max_pool], apply_random_aggregations=False, permute_index=[], pre_act_permute_gcn=False, post_act_permute_gcn=False):

        super(GCN_molecule_regression, self).__init__()

        self.permute_index = permute_index
        if len(permute_index) == 0:
            permute_index = list(range(gcn_layers))

        self.pre_act_permute_gcn = pre_act_permute_gcn
        self.post_act_permute_gcn = post_act_permute_gcn
    ...
    def forward(self, x, edge_index, batch_index):
        ...
        for iconv, conv in enumerate(self.convs):
            x = conv(x, edge_index)

            if self.pre_act_permute_gcn:
                if iconv in self.permute_index:
                    x = self.permute_features(x)


            x = self.activation1(x)


            if self.post_act_permute_gcn:
                if iconv in self.permute_index:
                    x = self.permute_features(x)
    ...
    def permute_features(self, x):
        batch_size, hidden_channels = x.size()
        permuted_indices = torch.randperm(hidden_channels)
...
class GCN_molecule_classification(torch.nn.Module):
    ...
    def __init__(self, input_dim, hidden_channels=[64], num_classes=1, gcn_layers=4, linear_sizes=[], aggregations=[global_mean_pool, global_max_pool], apply_random_aggregations=False, permute_index=[], pre_act_permute_gcn=False, post_act_permute_gcn=False):
